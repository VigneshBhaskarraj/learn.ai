...
| 🔊 **Listen & Conversation** | On-device text-to-speech reads any lesson aloud (iOS Safari + Android Chrome, free, offline). **Conversation mode** turns the lesson into a two-host, podcast-style dialogue read by two distinct device voices — a free approximation of NotebookLM-style audio |
| 📊 **Visual lessons** | Every one of the 54 lessons embeds a tailored infographic — stat strips, comparison tables, A-vs-B blocks, bar charts or numbered flows — so the material feels light and to-the-point, not a wall of text |
